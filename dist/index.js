...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"parser.ts":[function(require,module,exports) {
"use strict";
...
function dbg(expression) {
  var logParts = evaluateDbg(expression);
  formatAndLog(logParts);
  return logParts.value;
}

function evaluateDbg(expression) {
  if (typeof expression === 'function') {
    return {
      location: getCallerLocation(getStackTrace()),
      expression: (0, _parser.default)(expression.toString()),
      value: expression()
    };
  }
...
function getCallerLocation(stackTrace) {
  var LOCATION = 1;
  if (stackTrace.length < LOCATION + 1) return null;
  var matches = stackTrace[LOCATION].match(/(\/[\w-]+)?(\/[\w-]+[.]js:[\d]+):[\d]+/);
  if (matches === null) return null;
  var directory = matches[1] || '';
  var fileAndLine = matches[2] || '';
  return "".concat(directory).concat(fileAndLine);
} // location and expression are optional
// [location] expression = value
// [src/main.rs:2] a * 2 = 4


function formatAndLog(_ref) {
  var location = _ref.location,
      expression = _ref.expression,
      value = _ref.value;
  var parts = [];
  if (!!location) parts.push("[".concat(location, "]"));
...
},{"./parser":"parser.ts"}],"../../../../.config/yarn/global/node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
